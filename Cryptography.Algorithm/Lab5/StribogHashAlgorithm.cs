...
{
    public class StribogHashAlgorithm : CryptoAlgorithm
    {
        ...
        public StribogHashAlgorithm(int outputLenght)
        {
            if (outputLenght == 512)
            {
                for (int i = 0; i < 64; i++)
        ...
        {
            return ASCIIEncoding.ASCII.GetString(GetHash(ASCIIEncoding.ASCII.GetBytes(strToEncryption)));
        }

        public override string Decrypt(string strToDecryption)
        {
            throw new InvalidOperationException("Hash functions not support decryption.");
        }
    }
}
